...
init:
	...
	infra/scripts/confirm.sh $(TERRAFORM_ENVIRONMENT)
	...
	$(MAKE) -C infra/packages/cluster-disk-image init build
...
download-prod-env:
	...
	@  infra/scripts/download-prod-env.sh ${ENV}
...
update-prod-env:
	@ hcp auth login
	@ hcp profile init --vault-secrets
	@ infra/scripts/update-prod-env.sh ${ENV}
...
plan-without-jobs:
	@ printf "Planning Terraform for env: `tput setaf 2``tput bold`$(ENV)`tput sgr0`\n\n"
	$(eval TARGET := $(shell cat infra/main.tf | grep "^module" | awk '{print $$2}' | tr ' ' '\n' | grep -v -e "nomad" | awk '{print "-target=module." $$0 ""}' | xargs))
	$(tf_vars) \
	$(TF) plan \
	-out=.tfplan.$(ENV) \
	-input=false \
...
.PHONY: version
version:
	infra/scripts/increment-version.sh

.PHONY: build
build/%:
	$(MAKE) -C infra/packages/$(notdir $@) build
...
build-and-upload/template-manager:
	infra/scripts/confirm.sh $(TERRAFORM_ENVIRONMENT)
	GCP_PROJECT_ID=$(GCP_PROJECT_ID) $(MAKE) -C infra/packages/orchestrator build-and-upload/template-manager
build-and-upload/orchestrator:
	infra/scripts/confirm.sh $(TERRAFORM_ENVIRONMENT)
	GCP_PROJECT_ID=$(GCP_PROJECT_ID) $(MAKE) -C infra/packages/orchestrator build-and-upload/orchestrator
build-and-upload/api:
	infra/scripts/confirm.sh $(TERRAFORM_ENVIRONMENT)
	GCP_PROJECT_ID=$(GCP_PROJECT_ID) $(MAKE) -C infra/packages/api build-and-upload
	GCP_PROJECT_ID=$(GCP_PROJECT_ID) $(MAKE) -C infra/packages/db build-and-upload
build-and-upload/clickhouse-migrator:
	infra/scripts/confirm.sh $(TERRAFORM_ENVIRONMENT)
	GCP_PROJECT_ID=$(GCP_PROJECT_ID) $(MAKE) -C infra/packages/clickhouse build-and-upload
build-and-upload/%%:
	infra/scripts/confirm.sh $(TERRAFORM_ENVIRONMENT)
	GCP_PROJECT_ID=$(GCP_PROJECT_ID) $(MAKE) -C infra/packages/$(notdir $@) build-and-upload
...
generate/%%:
	...
	$(MAKE) -C infra/packages/$(notdir $@) generate
	@printf "\n\n"

.PHONY: migrate
migrate:
	$(MAKE) -C infra/packages/db migrate/up
...
.PHONY: test
test:
	$(MAKE) -C infra/packages/api test
	$(MAKE) -C infra/packages/client-proxy test
	$(MAKE) -C infra/packages/docker-reverse-proxy test
	$(MAKE) -C infra/packages/envd test
	$(MAKE) -C infra/packages/orchestrator test
	$(MAKE) -C infra/packages/shared test

.PHONY: test-integration
test-integration:
	$(MAKE) -C infra/tests/integration test

.PHONY: connect-orchestrator
connect-orchestrator:
	$(MAKE) -C infra/tests/integration connect-orchestrator

.PHONY: fmt
fmt:
	@infra/scripts/golangci-lint-install.sh "2.1.6"
	/home/kingubaish786/gopath\:/google/gopath/bin/golangci-lint fmt
...
.PHONY: generate-sdk
generate-sdk: generate-js generate-python
...
# Targets from code-interpreter/Makefile
start-template-server:
	docker run --rm -e E2B_LOCAL=true -p 49999:49999 -it $$(docker build . -q -f ./template.Dockerfile)
